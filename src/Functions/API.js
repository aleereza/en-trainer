...
const API = (year, race) => {
  ...
  let URL = `${baseURL}/${year}/${race}/results.json`;
  let data = fetch(URL)
    .then(resp => resp.json())
    .then(data => data.MRData)
    .catch(err => `${err} error happened!`);
  return data;
};

//todo: how use cache in api call?

export default API;

//API Client
//source: https://github.com/btholt/petfinder-client/blob/master/index.js
